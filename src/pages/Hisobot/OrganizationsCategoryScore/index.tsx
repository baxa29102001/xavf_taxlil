...
const Index = () => {
  ...
  const { isLoading } = useQuery(
    ["organizations-category-scores", searchValue, year, quarter],
    () =>
      axiosT.get<{ organizations: DataType[] }>(
        `/reports/organizations-category-scores/`,
        {
          params: {
            year: year,
            search: searchValue,
    ...
    <div>
      <div className={"grid grid-cols-12"}>
        <div className="col-span-6">
          <PageTitle
            title={"Xavf tahlili natijalari yo‘nalishlar kesimida"}
            back
          />
        </div>
      ...
      <div className={"grid grid-cols-12 gap-4 mb-5"}>
        ...
        <div className="col-span-6 flex justify-end">
          <Space>
            ...
            <RangePicker className="w-96" placeholder={["dan", "gacha"]} />
          </Space>
        </div>
      </div>
                          ...
                          {scores.category_scores.map(
                            (item: {
                              ...
                              if (item.category_id === category.category_id) {
                                return <div key={item.id}>{item.score}</div>;
                              }
                            }
                          )}
                        </div>
