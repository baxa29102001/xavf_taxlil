...
const Index = () => {
  ...
  const { isLoading: isLoadingOrganizations } = useQuery(
    ["organizations-select", categoryId],
    () =>
  ...
  useQuery(
    ["organizations-scores", searchValue, year, quarter, organizationId],
    () =>
      axiosT.get<{ regions: IOrganizationByRegion[] }>(
        `/reports/organization-scores-by-region/`,
        {
          params: {
            year: year,
            search: searchValue,
  ...
  useEffect(() => {
    ...
    const uniqueItems = unnested.filter(
      (item, index, self) =>
      ...
      <div className={"grid grid-cols-12 gap-4 mb-5"}>
        ...
        <div className="col-span-6 flex justify-end">
          <Space>
            <Select
              allowClear
              placeholder="Tadbirkorlik subyektlari"
            ...
            <RangePicker placeholder={["dan", "gacha"]} />
          </Space>
        </div>
      </div>
                ...
                {dataSource?.map((item) => {
                  return (
                    <th
                      key={item.region_id}
                      className={
                        "px-3 py-3 bg-[#fafafa] border-r border-r-[#f0f0f0]"
                      }
                    >
                      {item.region}
                    </th>
              ...
              {Criteries.map((criteries) => {
                return (
                  <tr key={criteries.name}>
                    <td
                      className={
                        "py-3 px-3 w-[300px] bg-white border-b border-[#f0f0f0]"
                      }
                    >
                      {criteries.name}
                    </td>
                    {dataSource.map((score) => {
                      return (
                        <td
                          key={score.region_id}
                          className={
                            "bg-white text-center  border-b border-[#f0f0f0]"
                          }
                        >
                          {score.criteria_scores.map((item) => {
                            if (
                              item.region_id === score.region_id &&
                              criteries.name === item.name
                            ) {
